...
def countPeopleOnCruise(ship_data, cruise_name, cruise_time):
    ...
    for cruise_data in ship_data:
        if cruise_data[2] == cruise_time and cruise_data[4] == cruise_name:
            total_people = total_people + cruise_data[5]
    return total_people
...
if (selected == "Strona główna"):
    ...
    for new_row in c.fetchall():
        dane = [f"Imię i nazwisko: {new_row[1]}", f"Numer telefonu: {new_row[12]} {new_row[6]}", f"{new_row[2]}", f"Statek: {new_row[3]}", f"{new_row[8]}" , new_row[7], f"Zaliczka: {new_row[4]}", f"Kwota zaliczki: {new_row[9]} zł", f"Bilet opłacony: {new_row[5]}", f"Notatki: {new_row[10]}", new_row[0]]
